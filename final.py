...
def display_chatbot():
    ...
    with st.sidebar.expander("💬 Legal Chatbot", expanded=False):
        ...
        st.text_area("Chat History", chat_history_text, height=200, disabled=True)

        # Chat input field
        # Use the counter to generate a unique key
        unique_key = f"chat_input_{st.session_state.chat_input_counter}"
        st.session_state.chat_input_counter += 1  # Increment the counter

        # Chat input field
        user_query = st.text_input("Ask a legal question:", key=unique_key)
        # Submit button
        if st.button("Send"):
            if user_query.strip():  # Only process if input is not empty
                with st.spinner("Getting response..."):
                    # Add context based on the current page
                    context = ""
                    if st.session_state.page == "Document Analysis" and st.session_state.document_text:
                        context = f"Context: The user has uploaded a document with the following text:\n{st.session_state.document_text[:500]}...\n"
                    elif st.session_state.page == "Risk Assessment" and st.session_state.risk_assessment:
                        context = f"Context: The user has performed a risk assessment with the following results:\n{st.session_state.risk_assessment[:500]}...\n"
                    elif st.session_state.page == "Document Generator" and "generated_doc" in st.session_state:
                        context = f"Context: The user has generated a document with the following content:\n{st.session_state.generated_doc[:500]}...\n"

                    # Get chatbot response
                    response = get_chatbot_response(user_query, st.session_state.language, context=context)
                    st.session_state.chat_history.append((user_query, response))  # Store chat history
...
if page == "Document Analysis":
    ...
    # Initialize last_query if not in session state
    ...
    # Display chat history as a text area
    chat_history_text = "\n".join([f"You: {q}\nAI: {a}\n" for q, a in st.session_state.chat_history])
    st.text_area("Chat History", chat_history_text, height=300, disabled=True)

    # Add the "Clear Chat History" button here with a unique key
    if st.button("Clear Chat History", key="clear_chat_history"):
        st.session_state.chat_history = []  # Clear the chat history
        st.success("Chat history cleared successfully!")  # Optional confirmation message

    # Chat input field (use a unique key each time to reset the input)
    user_query = st.text_input("Ask a legal question:", key=f"chat_input_{len(st.session_state.chat_history)}")
    ...
    with col1:
        ...
        if st.button(button_label, key="voice_button"):
            st.session_state.is_recording = not st.session_state.is_recording

            # Here we would typically handle the recording logic
            if not st.session_state.is_recording:  # Just stopped recording
                # Placeholder for speech recognition integration
                # In a real implementation, you would process the audio here
                # and set user_query to the transcribed text
                st.info("Speech recognition would be processed here.")

                # For demonstration, let's assume we got some text from speech recognition
                # In a real implementation, replace this with actual speech-to-text conversion
                # user_query = process_speech_to_text()

                # Add JavaScript for browser-based speech recognition
                st.markdown("""
                <script>
    ...
    with col2:
        # Add a button to read the latest response aloud
        if st.button("Read Last Response Aloud", key="text_to_speech_button"):
            if st.session_state.chat_history:
                last_response = st.session_state.chat_history[-1][1]
                # Here you would integrate text-to-speech functionality
                st.info("Text-to-speech would play the last response.")

                # Add JavaScript for browser-based text-to-speech
                js_code = f"""
                <script>
                    if ('speechSynthesis' in window) {{
                        const speech = new SpeechSynthesisUtterance('{last_response}');
    ...
    if st.button("Send", key="send_button"):
        if user_query.strip():  # Only process if input is not empty
            if st.session_state.last_query != user_query:  # Ensure it's a new query
                st.session_state.last_query = user_query  # Store last query
                with st.spinner("Getting response..."):
    ...
    if selected_doc != "Others":
        ...
        custom_query = st.text_area("Describe your document requirements:", height=150, placeholder="e.g., I need a custom agreement for a freelance project...")
    ...
    st.markdown("""
        ...
        """, unsafe_allow_html=True)
    research_query = st.text_area("Describe the legal issue or question you want to research:", height=100)
    ...
    if "research_results" in st.session_state:
        ...
        with col2:
            pdf_buffer = convert_markdown_to_pdf(edited_research, "Legal Research Results")
            if pdf_buffer:
                st.download_button(
                    label="📑 Download as PDF",
                    data=pdf_buffer,
                    file_name="legal_research.pdf",
                    mime="application/pdf",
                    use_container_width=True
                )
        img = render_text_as_image(edited_research, "Legal Research Results")
        if img:
            st.markdown(
                get_image_download_link(img, "legal_research.png", "📊 Download as Image"),
                unsafe_allow_html=True
            )
    ...
    <div class="footer">
        ...
        <p>© 2025 AI Legal Assistant | Developed with Streamlit and Google Gemini AI</p>
        <p><small>Disclaimer: This tool provides information for general purposes only and is not a substitute for professional legal advice.</small></p>
    </div>
    """, unsafe_allow_html=True)
    # Add a help section
    with st.expander("ℹ️ Help & About"):
        ...
        This application uses advanced AI to assist with various legal tasks:
        - **Document Analysis**: Extract and analyze text from legal documents
        - **Legal Chatbot**: Get answers to your legal questions
        - **Risk Assessment**: Identify potential risks in your contracts
        - **Document Generator**: Create customized legal documents
        ...
        - The AI provides general information and may not account for jurisdiction-specific laws
        ### Troubleshooting
        - If PDF/image processing fails, make sure you have the required dependencies installed
        - For best results, ensure uploaded documents are clearly scanned/photographed
        - If you encounter errors, try refreshing the page or uploading a different document format
        """)
    display_chatbot()
